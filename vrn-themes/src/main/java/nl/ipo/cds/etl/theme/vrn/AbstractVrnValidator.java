...
public class AbstractVrnValidator  extends AbstractValidator<AbstractGebied, Message, Context>
{
	...
	/**
	 * IMNa validatie op attribuut, type, kardinaliteit
	 *  (xsd validatie)
	 */
	...
	public Validator<Message, Context> getInspireIdLocalIdValidator () {
		return validate (
			and(
				validate (not (inspireIdLocalId.isNull ())).message (Message.ATTRIBUTE_NULL, constant(inspireIdLocalId.name)),
				validate (not (isBlank (inspireIdLocalId))).message (Message.ATTRIBUTE_EMPTY, constant(inspireIdLocalId.name))
			).shortCircuit()
		);
	}
